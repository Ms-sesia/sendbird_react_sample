 ...
 */
const SbCallsProvider = ({ appId, children }: { appId: string; children: ReactElement }): JSX.Element => {
  ...
  const selectAudioInputDevice = useCallback<ContextType["selectAudioInputDevice"]>((mediaInfo: InputDeviceInfo) => {
    SendbirdCall.selectAudioInputDevice(mediaInfo);
    dispatch({ type: "UPDATE_AUDIO_INPUT_DEVICE_INFO", payload: { current: mediaInfo } });
  }, []);

  const selectAudioOutputDevice = useCallback<ContextType["selectAudioOutputDevice"]>((mediaInfo: MediaDeviceInfo) => {
    SendbirdCall.selectAudioOutputDevice(mediaInfo);
    dispatch({ type: "UPDATE_AUDIO_OUTPUT_DEVICE_INFO", payload: { current: mediaInfo } });
  }, []);

  const selectVideoInputDevice = useCallback<ContextType["selectVideoInputDevice"]>((mediaInfo: InputDeviceInfo) => {
